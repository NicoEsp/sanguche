-- La guarda mira el vínculo con LemonSqueezy, no el plan local, y la fila se
-- bloquea mientras se decide.
--
-- Review de CodeRabbit sobre la migración anterior:
-- 1. La guarda exigía plan premium/repremium. "Pasar a Free" conserva el id
--    de LemonSqueezy y deja status active, así que una asignación manual
--    posterior la esquivaba y limpiaba el id con la suscripción todavía
--    cobrando. Ahora bloquea cualquier fila con id de suscripción y status
--    distinto de cancelled (active o inactive: en LemonSqueezy paused y
--    past_due siguen vivas y pueden volver a cobrar).
-- 2. Para que "Pasar a Free" no deje vínculos muertos que después bloqueen,
--    al pasar a Free se limpian los ids si la suscripción ya está cancelada
--    en LemonSqueezy; si sigue vigente se conservan, y la fila queda
--    bloqueada para asignaciones manuales hasta que se cancele allá. En
--    producción hoy no hay filas free con id de suscripción.
-- 3. El SELECT toma FOR UPDATE, así el webhook no puede cambiar la fila entre
--    la guarda y el upsert. Para el caso sin fila (el webhook podría
--    insertarla en el medio) el upsert vuelve a chequear el vínculo y, si
--    apareció uno vigente, no toca nada y la función falla pidiendo reintentar.
