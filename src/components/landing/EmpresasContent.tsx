...
interface EmpresasContentProps {
  /** El botón de checkout depende de useAuth, así que lo inyecta la página. */
 ...
 * scripts/prerender/render.tsx): el HTML servido de /empresas no traía nada
 * dentro del #root, así que un fetcher que no ejecuta JS no veía el programa.
 *
 * Lo único que entra por props es el botón de compra, que depende de useAuth y
 * por eso no puede correr en el build. El selector de temario sí se queda acá:
 * renderToStaticMarkup lo renderiza con su estado inicial, que es el mismo
 * primer pintado que ve cualquiera al entrar, no una versión para crawlers.
 */
export function EmpresasContent({ checkoutSlot }: EmpresasContentProps) {
