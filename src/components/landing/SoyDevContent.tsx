...
interface SoyDevContentProps {
  /** A dónde lleva el CTA de cada evaluación: depende de si hay sesión. */
  assessmentLink: (tipo: AssessmentTypeKey) => { to: string; state?: { from: { pathname: string } } };
  onAssessmentClick: (tipo: AssessmentTypeKey) => void;
}

/**
 * Cuerpo de /soy-dev.
 *
 * Vive fuera de la página para que lo pueda renderizar también el build (ver
 * scripts/prerender/render.tsx): el HTML servido de /soy-dev no traía nada
 * dentro del #root, así que un fetcher que no ejecuta JS no veía ni el título.
 *
 * Sin useAuth a propósito: ese hook arrastra el cliente de Supabase, que toca
 * localStorage al importarse y rompe en Node. Lo que depende de la sesión —el
 * destino del CTA— y el tracking entran por props, así el mismo markup sirve
 * para la página y para el HTML estático, sin copy duplicado que pueda divergir.
 */
export function SoyDevContent({ assessmentLink, onAssessmentClick }: SoyDevContentProps) {
  return (
        ...
        <section className="container py-12 sm:py-20 px-4 sm:px-6 relative overflow-hidden">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            ...
            >
              <Link
                ...
                onClick={() => onAssessmentClick("sin_experiencia")}
              >
          ...
          <p className="mt-4 text-center text-sm text-muted-foreground">
            <Link
              ...
              onClick={() => onAssessmentClick("experimentado")}
                    ...
                    {cta && (
                      <Button asChild variant="outline" size="sm" className="mt-4 w-full sm:w-auto sm:self-start">
                        <Link
                          ...
                          onClick={() => onAssessmentClick(cta.tipo)}
                        >
                          {cta.label}
