...
import { AnyAssessmentValues, AssessmentResult, AssessmentTypeKey, OptionalAssessmentValues } from '@/utils/scoring';
...
/**
 * Query de la última evaluación. La usan el hook y el prefetch del sidebar:
 * con un queryFn copiado en cada lado, la copia del sidebar había quedado con
 * otra key y filtrando por el id equivocado.
 */
export const assessmentDataQuery = (userId: string | undefined) => ({
  queryKey: ['assessment-data', userId] as const,
  queryFn: async (): Promise<CachedAssessment | null> => {
